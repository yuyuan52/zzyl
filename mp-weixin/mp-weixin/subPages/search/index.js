  ...
  setup(__props) {
        ...
        if (res.code === 200) {
          ...
          itemData.value.map((item, index) => {
            if (nameVal) {
              ...
              itemData.value[index].strName = item.name.replace(
                replaceReg,
                replaceString
              );
            }
    ...
    const handleBlur = (e) => {
      ...
      if (text !== "") {
        ...
        console.log(searchHistoryData.value);
      }
    };
    ...
    const handleList = (val) => {
      ...
      if (searchData.value.name !== "") {
        let newStr = "";
        if (/\s/.test(searchData.value.name)) {
          newStr = searchData.value.name.replace(/ /g, "");
        } else {
          newStr = searchData.value.name;
        }
        if (newStr.length > 0) {
          val = newStr;
          common_vendor.index.setStorageSync("historyData", searchHistoryData.value);
    ...
    const handleSearch = () => {
      handleBlur(searchData.value.name);
      handleList(searchData.value.name);
    };
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        i: common_vendor.n(searchData.value.name !== "" ? "font-col" : ""),
        ...
        r: common_vendor.f(searchHistoryData.value, (item, index, i0) => {
          return {
            a: common_vendor.t(item.length > 10 ? item.substring(0, 10) + "..." : item),
            b: index,
            c: common_vendor.o(($event) => handleHistory(item), index)
          };
        })
      } : {}, {
        s: capsuleBottom.value + "px"
      }) : {
        t: common_vendor.p({
          handleToRefresh
        })
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-e87c614f"], ["__file", "D:/project/2024/project-zhyl-xcx-uniapp-java-hongbo-v2.0/project-zhyl-xcx-uniapp-java2.0/subPages/search/index.vue"]]);
