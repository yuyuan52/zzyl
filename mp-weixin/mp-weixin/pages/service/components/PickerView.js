...
const _sfc_main = {
  ...
  setup(__props, { expose, emit }) {
    ...
    common_vendor.onMounted(() => {
      for (let i = year.value; i <= date.getFullYear() + 1; i++) {
        years.value.push(i);
      }
      for (let i = 0; i <= 23; i++) {
        i = utils_index.addZero(i);
    ...
    let oldDay = common_vendor.ref(null);
    const bindChange = (e) => {
      ...
      if (y === yearData) {
        oldDay.value = days.value[val[2]];
        if (dayData && dayData > oldDay.value) {
          oldDay.value = dayData + 1;
        }
        if (m > mon) {
          ...
          months.value = [...monBasedata].splice(mon - 1);
          const isInclude = months.value.includes(m);
          let monDay = null;
        ...
        hours.value = hourBasedata;
        minutes.value = minBasedata;
        months.value.splice(mon);
        let monDay = null;
        if (m < mon) {
          days.value = [...data];
        } else {
          console.log(m, mon, month.value);
          const isInclude = months.value.includes(m);
          days.value = [...data].splice(0, oldDay.value ? oldDay.value : dayData + 1);
          if (!isInclude) {
            monDay = mon;
          } else {
            monDay = month.value;
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        m: common_vendor.sr(popup, "1c0f811c-0", {
          "k": "popup"
        })
      });
    };
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "D:/project/2024/project-zhyl-xcx-uniapp-java-hongbo-v2.0/project-zhyl-xcx-uniapp-java2.0/pages/service/components/PickerView.vue"]]);
